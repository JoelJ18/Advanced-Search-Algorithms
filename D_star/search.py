...
class DStar:
    ...
    def process_state(self):
        ...
        '''
        #### TODO ####
        n=self.min_node() # Pop node from open list
        if(n is None):    
            return -1

        k_old=n.k
        self.delete(n)    #Moves node to closed
        
        # Get neighbors of the node
        # using self.get_neighbors
        # If node k is smaller than h (RAISE)                                
        if(k_old<n.h):
            for i in self.get_neighbors(n):
                if(i.h<k_old and n.h>i.h+(self.cost(i,n))):
                    n.parent=i
                    n.h=i.h+(self.cost(i,n))

        # If node k is the same as h (LOWER)
        if(k_old==n.h):
            for i in self.get_neighbors(n):
                if(i.tag=="NEW" or (i.parent==n and i.h!=n.h+(self.cost(i,n))) or (i.parent!=n and i.h>n.h+(self.cost(i,n)))):
                    i.parent=n
                    self.insert(i,n.h+(self.cost(i,n)))

        # Else node k is smaller than h (RAISE)
        else:
            for i in self.get_neighbors(n):
                if(i.tag=="NEW" or (i.parent==n and i.h!=n.h+(self.cost(i,n)))):
                    i.parent=n
                    self.insert(i,n.h+(self.cost(i,n)))
                    

                else:
                    if(i.parent!=n and i.h>n.h+(self.cost(i,n))):
                        self.insert(n,n.h)

                    else:
                        if(i.parent!=n and (n.h>i.h+(self.cost(i,n))) and (i.tag=="CLOSED") and (i.h>k_old)):
                            self.insert(i,i.h)
    ...
    def prepare_repair(self, node):
        ...
        for i in self.get_neighbors(node):
            if(i.is_dy_obs==True and i.is_obs==False):
