    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            # Calculate center of the ellipsoid - x_center                
                xcenter=([(self.goal.row+self.start.row)/2],[(self.goal.col+self.start.col)/2])
            ...
            # Compute rotation matrix from elipse to world frame - C
                a=np.arctan2(self.goal.col-self.start.col,self.goal.row-self.start.row)
                C=([np.cos(a),-1*np.sin(a)],[np.sin(a),np.cos(a)])

            # Compute diagonal matrix - L
                r1=c_best/2
                r2=(math.sqrt(c_best**2 - cmin**2))/2
                L=([r1,0],[0,r2])
            ...
            # Map ball sample to the ellipsoid - x_rand
                ...
                if(xrand[0]<self.size_row and xrand[1]<self.size_col):
